export const fears_dialog_01 = [
  {
    key: '1',
    english: 'AAAAHHHH! LUCY! COME QUICK! => (Runs into the room) ',
    russian: 'ААААА! ЛЮСИ! ИДИ СКОРЕЕ! => (Вбегает в комнату)',
  },
  {
    key: '2',
    english: "What happened? Are you okay? => There's a spider! Over there, on the wall! It's huge! => (Looks) ",
    russian: 'Что случилось? Ты в порядке? => Там паук! Вон там, на стене! Он огромный! => (Смотрит)',
  },
  {
    key: '3',
    english: "That little thing? It's so small. I can barely see it. => Small? It's enormous! I hate spiders. I'm really scared of them.",
    russian: "Эта маленькая штучка? Он такой маленький. Я едва вижу. => Маленький? Он огромный! Ненавижу пауков. Я очень их боюсь.",
  },
  {
    key: '4',
    english: "Why? It's just a tiny spider. It can't hurt you. => I know it's silly, but I can't help it. ",
    russian: 'Почему? Это просто крошечный паук. Он не может тебе навредить. => Знаю, что глупо, но ничего не могу поделать.',
  },
  {
    key: '5',
    english: "My heart is beating so fast. => Okay, calm down. I'll take care of it. Where's a glass?",
    russian: 'Моё сердце так быстро бьётся. => Хорошо, успокойся. Я разберусь. Где стакан?',
  },
  {
    key: '6',
    english: "In the kitchen, top cupboard. Please be careful! => Don't worry. (Catches spider) - See? Easy. I'll put it outside.",
    russian: 'На кухне, в верхнем шкафу. Пожалуйста, будь осторожна! => Не волнуйся. (Ловит паука) - Видишь? Легко. Вынесу его на улицу.',
  },
  {
    key: '7',
    english: "Thank you so much. You're my hero. I couldn't sleep if it was still there.",
    russian: 'Большое спасибо. Ты мой герой. Я бы не уснула, если бы он всё ещё был там.',
  },
  {
    key: '8',
    english: "No problem. You know, my brother is scared of spiders too. It's very common.",
    russian: 'Без проблем. Знаешь, мой брат тоже боится пауков. Это очень распространено.',
  },
  {
    key: '9',
    english: 'Really? I thought I was the only one. Everyone laughs at me. => Lots of people are afraid of spiders.',
    russian: 'Правда? Я думала, я одна такая. Все смеются надо мной. => Многие люди боятся пауков.',
  },
  {
    key: '10',
    english: '--------------------------------------------------------',
    russian: '--------------------------------------------------------',
  },
  {
    key: '11',
    english: 'So, are you excited about your vacation? Italy sounds amazing! => I am excited, but also really nervous. I hate the flight.',
    russian: 'Итак, ты рада отпуску? Италия звучит потрясающе! => Я рада, но и очень нервничаю. Ненавижу перелёты.',
  },
  {
    key: '12',
    english: 'Oh no, are you scared of flying? => Terrified. The plane could crash, or there could be turbulence... I hate it all.',
    russian: 'О нет, ты боишься летать? => До ужаса. Самолёт может разбиться, или может быть турбулентность... Всё ненавижу.',
  },
  {
    key: '13',
    english: 'I understand. My mom is the same. But flying is actually very safe. => I know, I know. Everyone tells me that. ',
    russian: 'Понимаю. Моя мама такая же. Но летать на самом деле очень безопасно. => Знаю, знаю. Все мне это говорят.',
  },
  {
    key: '14',
    english: "But my brain doesn't believe it. => How long is the flight? - Only two hours. But two hours of fear feels like forever.",
    russian: 'Но мой мозг не верит. => Сколько лететь? - Всего два часа. Но два часа страха кажутся вечностью.',
  },
  {
    key: '15',
    english: 'Do you have anything to help? Like medicine or music? => I downloaded movies on my tablet. And I have gum to chew during takeoff.',
    russian: 'У тебя есть что-то, что помогает? Лекарства или музыка? => Я скачала фильмы на планшет. И у меня есть жвачка, чтобы жевать во время взлёта.',
  },
  {
    key: '16',
    english: "Good idea. Also, tell the flight attendant. They can help if you panic. => Really? They won't think I am stupid?",
    russian: "Хорошая идея. И ещё скажи бортпроводнику. Они могут помочь, если запаникуешь. => Правда? Они не подумают, что я глупая?"
  },
  {
    key: '17',
    english: "No, they deal with scared passengers all the time. It's their job. => That's actually really good advice. Thanks, Mark.",
    russian: 'Нет, они постоянно имеют дело с испуганными пассажирами. Это их работа. => Это действительно хороший совет. Спасибо, Марк.',
  },
  {
    key: '18',
    english: "You'll be fine. Just think about the pizza and gelato waiting for you in Italy!",
    russian: 'Всё будет хорошо. Просто думай о пицце и джелато, которые ждут тебя в Италии!',
  },
  {
    key: '19',
    english: "Haha, that helps! Okay, I'm a little less scared now.",
    russian: 'Ха-ха, это помогает! Хорошо, теперь мне немного меньше страшно.',
  },
  {
    key: '20',
    english: '--------------------------------------------------------',
    russian: '--------------------------------------------------------',
  },
  {
    key: '21',
    english: 'Okay Leo, time for bed. Go brush your teeth and put your pajamas on.',
    russian: 'Хорошо, Лео, время спать. Иди чистить зубы и надевай пижаму.',
  },
  {
    key: '22',
    english: 'Mom... can you leave the hallway light on tonight? => Again? Leo, you are 8 years old now. ',
    russian: 'Мам... можно оставить свет в коридоре включённым сегодня? => Снова? Лео, тебе уже 8 лет. ',
  },
  {
    key: '23',
    english: "Why are you still scared of the dark? => I don't know. When the light is off, I see shadows. They look like monsters.",
    russian: 'Почему ты всё ещё боишься темноты? => Не знаю. Когда свет выключен, я вижу тени. Они похожи на монстров.',
  },
  {
    key: '24',
    english: "There are no monsters, sweetie. You know that. It's just your imagination. => I know, but I still feel scared",
    russian: 'Там нет монстров, милый. Ты знаешь это. Это просто твоё воображение. => Знаю, но мне всё равно страшно.',
  },
  {
    key: '25',
    english: 'Can you check under my bed? => (Looks under bed) - See? No monsters. Just your old toys.',
    russian: 'Можешь проверить под кроватью? => (Смотрит под кроватью) - Видишь? Никаких монстров. Просто твои старые игрушки.',
  },
  {
    key: '26',
    english: 'What about the closet? Can you open it? => (Opens closet) - Empty. Just clothes. See? Nothing to be afraid of.',
    russian: 'А шкаф? Можешь открыть? => (Открывает шкаф) -  Пусто. Просто одежда. Видишь? Нечего бояться.',
  },
  {
    key: '27',
    english: "Okay... but can you leave the door open a little? => Fine. I'll leave it open. And your night light is on, right?",
    russian: 'Ладно... но можно оставить дверь приоткрытой? => Хорошо. Оставлю открытой. И твой ночник включён, да?',
  },
  {
    key: '28',
    english: "Yes, it's on. It helps a little. => Good. Now close your eyes and think about happy things. Like puppies or ice cream.",
    russian: 'Да, включён. Немного помогает. => Хорошо. Теперь закрой глаза и думай о приятных вещах. О щенках или мороженом.',
  },
  {
    key: '29',
    english: 'Okay. Goodnight, Mom. I love you. => Goodnight, my brave boy. I love you too. Sleep well.',
    russian: 'Хорошо. Спокойной ночи, мам. Я люблю тебя. => Спокойной ночи, мой храбрый мальчик. Я тоже тебя люблю. Спи хорошо.',
  },
];

export const fears_dialog_02 = [
  {
    key: '1',
    english: "Anna, I think I'm going to be sick. My stomach hurts so bad. => Oh no! Is it something you ate?",
    russian: 'Анна, кажется, меня сейчас вырвет. Живот так сильно болит. => О нет! Это что-то съел?',
  },
  {
    key: '2',
    english: "No, it's the presentation. I have to speak in front of the whole class in 10 minutes. => Are you nervous? You look really pale.",
    russian: 'Нет, это презентация. Мне нужно выступать перед всем классом через 10 минут. => Ты нервничаешь? Ты выглядишь очень бледным.',
  },
  {
    key: '3',
    english: "Nervous is an understatement. I'm terrified. I hate public speaking. => Lots of people do. It's very common.",
    russian: 'Нервничаю — это мягко сказано. Я в ужасе. Ненавижу публичные выступления. => Многие боятся. Это очень распространено.',
  },
  {
    key: '4',
    english: "Just pretend everyone is in their underwear! => That's a stupid tip. That would make me more nervous!",
    russian: 'Просто представь всех в нижнем белье! => Это глупый совет. Это заставит меня нервничать ещё больше!',
  },
  {
    key: '5',
    english: 'Haha, okay, bad idea. Just focus on me. Look at me the whole time. => Really? I can just look at you?',
    russian: 'Ха-ха, ладно, плохая идея. Просто сосредоточься на мне. Смотри на меня всё время. => Правда? Я могу просто смотреть на тебя?',
  },
  {
    key: '6',
    english: "Yes. I'll sit in the front row. Pretend you're just talking to me, not the whole class.",
    russian: 'Да. Я сяду в первом ряду. Представь, что ты просто разговариваешь со мной, а не со всем классом.',
  },
  {
    key: '7',
    english: "That might actually work. You won't laugh at me if I make a mistake?",
    russian: 'Это может сработать. Ты не засмеёшься, если я ошибусь?',
  },
  {
    key: '8',
    english: 'Of course not! And no one else will laugh either. Everyone is nervous for their own turn.',
    russian: 'Конечно нет! И никто другой не засмеётся. Все нервничают из-за своего выступления.',
  },
  {
    key: '9',
    english: "You're right. Okay, take deep breaths. In... out... in... out. => That's it. You know your topic. ",
    russian: 'Ты права. Хорошо, глубокий вдох. Вдох... выдох... вдох... выдох. => Вот так. Ты знаешь тему.',
  },
  {
    key: '10',
    english: "You practiced a lot. You'll be fine. => Thanks, Anna. You're a good friend. I think I can do this now.",
    russian: 'Ты много практиковался. Всё будет хорошо. => Спасибо, Анна. Ты хороший друг. Думаю, теперь я смогу.',
  },
  {
    key: '11',
    english: "Of course you can! Now go up there and show them what you've got!",
    russian: 'Конечно сможешь! А теперь иди и покажи им, на что ты способен!',
  },
  {
    key: '12',
    english: '--------------------------------------------------------',
    russian: '--------------------------------------------------------',
  },
  {
    key: '13',
    english: "Oh no... David, look. There's a big dog coming. And no leash! => Where? Oh, that golden retriever? He looks friendly.",
    russian: 'О нет... Дэвид, смотри. Идёт большая собака. И без поводка! => Где? О, тот золотистый ретривер? Выглядит дружелюбно.',
  },
  {
    key: '14',
    english: "I don't care. I'm scared of dogs. Even friendly ones. => Really? Why? => When I was little, a dog jumped on me and knocked me down. I was so scared.",
    russian: 'Мне всё равно. Я боюсь собак. Даже дружелюбных. => Правда? Почему? => Когда я была маленькой, собака прыгнула на меня и сбила с ног. Мне было так страшно.',
  },
  {
    key: '15',
    english: "Oh, I'm sorry. That makes sense. But this dog looks very calm. => I know it's probably fine. But my heart is racing. What should I do?",
    russian: 'О, мне жаль. Это объясняет. Но эта собака выглядит очень спокойной. => Знаю, что всё, наверное, нормально. Но сердце колотится. Что мне делать?',
  },
  {
    key: '16',
    english: "Just stand still. Don't run. Dogs love to chase things that run. => Okay, I won't run. What if he comes to me?",
    russian: 'Просто стой на месте. Не беги. Собаки любят гоняться за тем, что бежит. => Хорошо, не побегу. А если он подойдёт ко мне?',
  },
  {
    key: '17',
    english: 'Then just let him sniff you. Hold out your hand like this. See? Let him smell you. => Scared I do not know if I can.',
    russian: 'Тогда просто позволь ему обнюхать тебя. Протяни руку вот так. Видишь? Пусть понюхает. => Испуганно Не знаю, смогу ли.',
  },
  {
    key: '18',
    english: "The owner is coming. See? He's calling the dog. It's okay. => Sorry! He got away from me. He's friendly, don't worry.",
    russian: 'Хозяин идёт. Видишь? Он зовёт собаку. Всё хорошо. => Извините! Он убежал от меня. Он дружелюбный, не волнуйтесь.',
  },
  {
    key: '19',
    english: "It's okay. I'm just a little scared of dogs. => Oh, I understand. Max, come here. Sorry again!",
    russian: 'Всё нормально. Я просто немного боюсь собак. => О, понимаю. Макс, иди сюда. Ещё раз извините!',
  },
  {
    key: '20',
    english: "See? It's fine. You did great. You didn't panic. => My hands are shaking, but I'm okay. Thanks for helping me.",
    russian: 'Видишь? Всё хорошо. Ты отлично справилась. Ты не запаниковала. => У меня руки дрожат, но я в порядке. Спасибо, что помог.',
  },
  {
    key: '21',
    english: '--------------------------------------------------------',
    russian: '--------------------------------------------------------',
  },
  {
    key: '22',
    english: "Let's take the elevator. It's faster. => Not the glass one! Please, let's take the stairs.",
    russian: 'Давай на лифте. Быстрее. => Только не стеклянный! Пожалуйста, давай по лестнице.',
  },
  {
    key: '23',
    english: 'Why? The glass elevator is cool. You can see the whole mall. => That is exactly the problem. I can see the whole mall... from way up high.',
    russian: 'Почему? Стеклянный лифт это круто. Видно весь торговый центр. => В этом-то и проблема. Я вижу весь торговый центр... с большой высоты.',
  },
  {
    key: '24',
    english: 'Oh, are you scared of heights? => Very scared. Even thinking about it makes me dizzy.',
    russian: 'О, ты боишься высоты? => Очень боюсь. Даже мысль об этом вызывает головокружение.',
  },
  {
    key: '25',
    english: "But we're only going to the third floor. That's not even high. => It's high enough for me. I'll wait here. You go. I'll take the stairs.",
    russian: 'Но мы всего на третий этаж. Это даже не высоко. => Для меня достаточно высоко. Я подожду здесь. Иди. Я по лестнице.',
  },
  {
    key: '26',
    english: "Come on, Emily. Face your fear! I'll hold your hand. => No way. Absolutely not. I can't.",
    russian: 'Давай, Эмили. Посмотри страху в лицо! Я буду держать тебя за руку. => Ни за что. Точно нет. Не могу.',
  },
  {
    key: '27',
    english: "Just try. One floor. If you don't like it, we'll never take it again. => Thinks... Okay, fine. But you have to hold my hand the whole time.",
    russian: 'Просто попробуй. Один этаж. Если не понравится, никогда больше не поедем. => Думает... Ладно, хорошо. Но ты должен держать меня за руку всё время.',
  },
  {
    key: '28',
    english: "Deal! Let's go. => Oh my god. Oh my god. Don't look down. Don't look down.",
    russian: 'Договорились! Пошли. => О боже. О боже. Не смотри вниз. Не смотри вниз.',
  },
  {
    key: '29',
    english: "Look at me. Just look at me. See? We're almost there. => Doors open We made it! I can't believe it. My legs are shaking",
    russian: 'Смотри на меня. Просто смотри на меня. Видишь? Мы почти на месте. => Двери открываются Мы сделали это! Не могу поверить. У меня ноги дрожат.',
  },
  {
    key: '30',
    english: "You did it! See? It wasn't so bad. => It was terrible. But I did it. Okay, never again. Stairs from now on!",
    russian: 'Ты сделала это! Видишь? Было не так уж плохо. => Это было ужасно. Но я сделала это. Ладно, никогда больше. Теперь только лестница!',
  },
];

export const fears_dialog_03 = [
  {
    key: '1',
    english: 'Loud thunder AHHH! (Jumps) => Whoa! Rachel, you scared me! ',
    russian: 'Громкий гром ААА! (Подпрыгивает) => Ого! Рейчел, ты меня напугала! ',
  },
  {
    key: '2',
    english: "What's wrong? => Sorry, sorry. It's just the thunder. I hate thunderstorms.",
    russian: 'Что случилось? => Извини, извини. Просто гром. Ненавижу грозы.',
  },
  {
    key: '3',
    english: "Really? I love them. They're so cozy. Rain and thunder... perfect sleeping weather.",
    russian: 'Правда? Я люблю их. Это так уютно. Дождь и гром... идеальная погода для сна.',
  },
  {
    key: '4',
    english: 'Not for me. When I was a kid, a tree near my house got hit by lightning. I was so scared.',
    russian: 'Не для меня. Когда я была ребёнком, в дерево возле дома ударила молния. Мне было так страшно.',
  },
  {
    key: '5',
    english: "Oh wow, that's actually scary. I understand why you're afraid.",
    russian: 'Ого, это действительно страшно. Понимаю, почему ты боишься.',
  },
  {
    key: '6',
    english: 'More thunder See? Every time it thunders, I remember that night.',
    russian: 'Ещё гром Видишь? Каждый раз, когда гремит гром, я вспоминаю ту ночь.',
  },
  {
    key: '7',
    english: 'Do you want to watch TV? It might distract you. => Good idea. Something funny. Not a horror movie!',
    russian: 'Хочешь посмотреть телевизор? Это может отвлечь. => Хорошая идея. Что-то смешное. Не ужастик!',
  },
  {
    key: '8',
    english: "Haha, don't worry. Let's put on a comedy show. => Thanks, Ben. You're nice to stay with me.",
    russian: 'Ха-ха, не волнуйся. Давай включим комедийное шоу. => Спасибо, Бен. Мило, что остаёшься со мной.',
  },
  {
    key: '9',
    english: 'No problem. Also, did you know? You can count the seconds between lightning and thunder?',
    russian: 'Без проблем. И ещё, ты знала? Можно считать секунды между молнией и громом?',
  },
  {
    key: '10',
    english: 'Why? => To know how far the storm is. Five seconds = one mile.',
    russian: 'Зачем? => Чтобы узнать, как далеко гроза. Пять секунд = одна миля.',
  },
  {
    key: '11',
    english: "Really? Okay, let's try. Lightning One... two... three... Thunder",
    russian: 'Правда? Хорошо, давай попробуем. Молния Раз... два... три... Гром',
  },
  {
    key: '12',
    english: "Three seconds. That's pretty close. About half a mile. => That's actually interesting. It makes it less scary somehow.",
    russian: 'Три секунды. Довольно близко. Около полумили. => Это действительно интересно. Почему-то это делает грозу менее страшной',
  },
  {
    key: '13',
    english: "See? Science helps with fear. Now let's watch TV and forget about the storm.",
    russian: 'Видишь? Наука помогает со страхом. А теперь давай смотреть телевизор и забудем о грозе.',
  },
  {
    key: '14',
    english: 'Sounds perfect. Thanks for being a good roommate.',
    russian: 'Звучит идеально. Спасибо, что ты хороший сосед.',
  },
  {
    key: '15',
    english: '--------------------------------------------------------',
    russian: '--------------------------------------------------------',
  },
  {
    key: '16',
    english: "I can't do this. I'm leaving. => What? Chris, we've been waiting for an hour. Your name is next!",
    russian: 'Я не могу этого сделать. Я ухожу. => Что? Крис, мы ждали час. Ты следующий!',
  },
  {
    key: '17',
    english: "I don't care. I can't get a shot. I'm terrified of needles.",
    russian: 'Мне всё равно. Я не могу сделать укол. Ужасно боюсь игл.',
  },
  {
    key: '18',
    english: "Really? But it's just a small pinch. It's over in seconds.",
    russian: 'Правда? Но это просто маленький укус. Всё кончится через секунды.',
  },
  {
    key: '19',
    english: "I know it's silly. But when I see the needle, I feel like I'm going to faint.",
    russian: 'Знаю, что глупо. Но когда я вижу иглу, мне кажется, я упаду в обморок.',
  },
  {
    key: '20',
    english: 'Oh no. Have you always been like this? => Yes, since I was a kid. My mom had to hold me down for shots.',
    russian: 'О нет. Ты всегда таким был? => Да, с детства. Маме приходилось держать меня для уколов.',
  },
  {
    key: '21',
    english: "Okay, don't panic. Look away. Don't watch. => I try, but I always look. It's like I can't help it.",
    russian: 'Хорошо, не паникуй. Отвернись. Не смотри. => Я пытаюсь, но всегда смотрю. Не могу с собой справиться.',
  },
  {
    key: '22',
    english: 'Close your eyes. And squeeze my hand really hard. Think about something else.',
    russian: 'Закрой глаза. И сожми мою руку очень сильно. Думай о чём-то другом.',
  },
  {
    key: '23',
    english: "Chris? We're ready for you. => Panicking Oh no. Oh no. Here we go.",
    russian: 'Крис? Мы готовы. => Паникуя О нет. О нет. Ну, пошли.',
  },
  {
    key: '24',
    english: "I'll come with you. I'll hold your hand and talk to you. Okay? => Okay. Don't let go.",
    russian: 'Я пойду с тобой. Буду держать за руку и разговаривать. Хорошо? => Хорошо. Не отпускай.',
  },
  {
    key: '25',
    english: "Look at me, Chris. Don't look at the nurse. Tell me about your vacation plans.",
    russian: 'Смотри на меня, Крис. Не смотри на медсестру. Расскажи о планах на отпуск.',
  },
  {
    key: '26',
    english: "I'm... I'm going to the beach next month. => Nice! Which beach? => Feels pinch OW!",
    russian: 'Я... Я еду на пляж в следующем месяце. => Круто! На какой пляж? => Чувствует укол ОЙ!',
  },
  {
    key: '27',
    english: 'Is it done? => All done! You were very brave. => That is it? It was not that bad.',
    russian: 'ОЙ! Всё? => Всё готово! Вы были очень храбрым. => И всё? Было не так уж плохо.',
  },
  {
    key: '28',
    english: "See? You did it! I'm proud of you. => Thanks, Anna. I couldn't have done it without you.",
    russian: 'Видишь? Ты сделал это! Я горжусь тобой. => Спасибо, Анна. Я бы не справился без тебя.',
  },
];

export const fears_dialog_04 = [
  {
    key: '1',
    english: "I'm so stressed about tomorrow's exam. I can't sleep, I can't eat.",
    russian: 'Я так переживаю из-за завтрашнего экзамена. Не могу спать, не могу есть.',
  },
  {
    key: '2',
    english: "You? But you're the best student in class. You always get As.",
    russian: 'Ты? Но ты лучшая ученица в классе. У тебя всегда пятёрки.',
  },
  {
    key: '3',
    english: "That's the problem. Everyone expects me to do well. What if I fail?",
    russian: 'В этом и проблема. Все ждут, что я хорошо сдам. А если я провалюсь?',
  },
  {
    key: '4',
    english: "You won't fail. You studied so much. You know everything.",
    russian: 'Ты не провалишься. Ты так много занималась. Ты всё знаешь.',
  },
  {
    key: '5',
    english: 'But what if the questions are hard? What if my mind goes blank?',
    russian: 'Но что, если вопросы будут сложными? Что, если в голове будет пусто?',
  },
  {
    key: '6',
    english: "That's just fear talking. You're afraid of failure. It's normal. => Is it? I feel like I'm going to be sick.",
    russian: 'Это просто страх говорит. Ты боишься неудачи. Это нормально. => Правда? Меня сейчас стошнит.',
  },
  {
    key: '7',
    english: "Lots of people feel this way before big exams. It's called atychiphobia.",
    russian: 'Многие так чувствуют перед важными экзаменами. Это называется атихифобия.',
  },
  {
    key: '8',
    english: 'Atychi-what? => Fear of failure. My brother has it. He almost did not take his driving test.',
    russian: 'Атихи-что? => Боязнь неудачи. У моего брата это было. Он чуть не отказался от экзамена по вождению.',
  },
  {
    key: '9',
    english: 'Really? What did he do? => He talked to a counselor. She said to think: what is the worst that can happen?',
    russian: 'Правда? И что он сделал? => Поговорил с консультантом. Она сказала подумать: что самое худшее может случиться?',
  },
  {
    key: '10',
    english: 'The worst? I fail the exam. => And then? Do you die? Does your life end?',
    russian: 'Самое худшее? Я провалю экзамен. => И потом? Ты умрёшь? Твоя жизнь закончится?',
  },
  {
    key: '11',
    english: 'No... I just take it again. => Exactly. Failing is not the end of the world. It is just a learning experience.',
    russian: 'Нет... Просто пересдам. => Именно. Провал — это не конец света. Это просто опыт.',
  },
  {
    key: '12',
    english: 'That actually helps. So if I fail, I try again. No big deal.',
    russian: 'Это действительно помогает. Если провалю, попробую снова. Не страшно.',
  },
  {
    key: '13',
    english: "Right. But you won't fail. You're ready. Just do your best. => Thanks, David. I feel much better now.",
    russian: 'Верно. Но ты не провалишься. Ты готова. Просто сделай всё возможное. => Спасибо, Дэвид. Теперь мне намного лучше.',
  },
  {
    key: '14',
    english: '--------------------------------------------------------',
    russian: '--------------------------------------------------------',
  },
  {
    key: '15',
    english: 'You look sad today. Everything okay? => I have a dentist appointment after work. I am dreading it.',
    russian: 'Ты выглядишь грустной сегодня. Всё в порядке? => У меня приём у стоматолога после работы. С ужасом жду этого.',
  },
  {
    key: '16',
    english: 'Oh no, are you scared of the dentist? => Terrified. The sounds, the smells, the tools... I hate it all.',
    russian: 'О нет, ты боишься стоматолога? => До ужаса. Звуки, запахи, инструменты... Всё ненавижу.',
  },
  {
    key: '17',
    english: "I used to be scared too. But I found a nice dentist. She's very gentle.",
    russian: 'Я тоже боялся. Но нашёл хорошего стоматолога. Она очень нежная.',
  },
  {
    key: '18',
    english: 'My dentist is okay. But the drill sound makes me want to run away.',
    russian: 'Мой стоматолог нормальный. Но звук бормашины заставляет меня хотеть убежать.',
  },
  {
    key: '19',
    english: 'Do you have a cavity or just a check-up? => A check-up. But last time they found two cavities. I am scared they will find more.',
    russian: 'У тебя кариес или просто осмотр? => Осмотр. Но в прошлый раз нашли две дырки. Боюсь, что найдут ещё.',
  },
  {
    key: '20',
    english: 'The more you go, the less they find. Regular check-ups prevent problems.',
    russian: 'Чем чаще ходишь, тем меньше находят. Регулярные осмотры предотвращают проблемы.',
  },
  {
    key: '21',
    english: "I know. But knowing that doesn't help my fear. => Ask them to explain everything before they do it. That helps me.",
    russian: 'Знаю. Но знание не помогает моему страху. => Попроси их объяснять всё перед тем, как делать. Мне это помогает.',
  },
  {
    key: '22',
    english: "Really? => Yes. When I know what's coming, I'm less scared. And bring headphones. Listen to music.",
    russian: 'Правда? => Да. Когда я знаю, что будет, мне меньше страшно. И возьми наушники. Слушай музыку.',
  },
  {
    key: '23',
    english: "That's a great idea. I'll download my favorite playlist. => And remember: it's only 30 minutes. Then it's over and you get a sticker!",
    russian: 'Отличная идея. Скачаю свой любимый плейлист. => И помни: всего 30 минут. Потом всё кончится, и ты получишь наклейку!',
  },
  {
    key: '24',
    english: "Haha, a sticker? I'm not 5 years old! => My dentist gives stickers to adults too. It's silly but it makes me happy.",
    russian: 'Ха-ха, наклейку? Мне не 5 лет! => Мой стоматолог даёт наклейки и взрослым. Глупо, но меня это радует.',
  },
  {
    key: '25',
    english: 'Okay, you made me smile. Thanks, Tom. I feel a little better.',
    russian: 'Ладно, ты меня рассмешил. Спасибо, Том. Мне немного лучше.',
  },
];

export const fears_dialog_05 = [
  {
    key: '1',
    english: 'Come on, Sarah! The water is perfect. Get in! => I... I do not think I can. I am scared.',
    russian: 'Давай, Сара! Вода идеальная. Заходи! => Я... Не думаю, что смогу. Мне страшно',
  },
  {
    key: '2',
    english: "Scared of what? The water is clean and warm. It's not deep here.",
    russian: 'Чего боишься? Вода чистая и тёплая. Здесь не глубоко.',
  },
  {
    key: '3',
    english: 'I never learned to swim. When I was a kid, I almost drowned in a pool.',
    russian: 'Я никогда не училась плавать. Когда я была маленькой, чуть не утонула в бассейне.',
  },
  {
    key: '4',
    english: "Oh no, that's terrible. I'm sorry. I understand why you're scared. => Everyone says swimming is fun, but I just panic in water.",
    russian: 'О нет, это ужасно. Мне жаль. Понимаю, почему ты боишься. => Все говорят, плавать весело, но я просто паникую в воде.',
  },
  {
    key: '5',
    english: "Do you want to try? Just in the shallow end? I'll hold your hand. => I don't know...",
    russian: 'Хочешь попробовать? Только на мелкой части? Я буду держать тебя за руку. => Не знаю...',
  },
  {
    key: '6',
    english: "We'll go slowly. Just sit on the edge and put your feet in first. => Puts feet in Okay. That's not so bad.",
    russian: 'Будем двигаться медленно. Просто сядь на край и сначала опусти ноги. => Опускает ноги Хорошо. Не так уж плохо.',
  },
  {
    key: '7',
    english: "Good! Now, I'll stand next to you. Just hold the edge and go down one step.",
    russian: 'Хорошо! Теперь я встану рядом. Держись за край и спустись на одну ступеньку.',
  },
  {
    key: '8',
    english: 'Goes down one step Water is up to my waist now. My heart is beating fast.',
    russian: 'Спускается на одну ступеньку Вода теперь до пояса. Сердце быстро бьётся.',
  },
  {
    key: '9',
    english: "You're doing great. Just breathe. In and out. Feel the water. => It's... actually nice. Warm.",
    russian: 'У тебя отлично получается. Просто дыши. Вдох и выдох. Почувствуй воду. => Она... действительно приятная. Тёплая.',
  },
  {
    key: '10',
    english: "See? You're in the water and you're safe. I'm right here. => This is the first time I've been in a pool in 10 years.",
    russian: 'Видишь? Ты в воде, и ты в безопасности. Я рядом. => Я впервые в бассейне за 10 лет.',
  },
  {
    key: '11',
    english: "I'm so proud of you. Maybe one day you can take swimming lessons? => Maybe. For now, this is enough. Thank you, Mike. You're very patient.",
    russian: 'Я так горжусь тобой. Может, однажды ты сможешь брать уроки плавания? => Может. Пока достаточно. Спасибо, Майк. Ты очень терпеливый.',
  },
  {
    key: '12',
    english: "That's what friends are for!",
    russian: 'На то и друзья!',
  },
  {
    key: '13',
    english: '--------------------------------------------------------',
    russian: '--------------------------------------------------------',
  },
  {
    key: '14',
    english: 'Hey Kate, are you busy? Can we talk? => Hi Anna! No, I am not busy. Just watching TV.',
    russian: 'Привет, Кейт, ты занята? Можем поговорить? => Привет, Анна! Нет, не занята. Просто смотрю телевизор.',
  },
  {
    key: '15',
    english: "What's up? => My roommate is away for the weekend. I'm alone in the apartment and I can't sleep.",
    russian: 'Что случилось? => Моя соседка уехала на выходные. Я одна в квартире и не могу уснуть.',
  },
  {
    key: '16',
    english: 'Oh no! Are you scared of being alone? => A little. Every little noise makes me jump.',
    russian: 'О нет! Ты боишься одиночества? => Немного. Каждый шорох заставляет меня вздрагивать',
  },
  {
    key: '17',
    english: 'I hear things and think someone is there. => That is scary. I do not like being alone at night either.',
    russian: 'Слышу звуки и думаю, что там кто-то есть. => Это страшно. Я тоже не люблю быть одна ночью.',
  },
  {
    key: '18',
    english: 'Really? I thought I was the only one. => No, lots of people do not like being alone. It is normal. We are social animals.',
    russian: 'Правда? Я думала, я одна такая. => Нет, многим не нравится быть одним. Это нормально. Мы социальные животные.',
  },
  {
    key: '19',
    english: 'What do you do when you feel scared? => I put on a TV show. Something funny. The voices make me feel less alone.',
    russian: 'Что ты делаешь, когда тебе страшно? => Включаю телешоу. Что-то смешное. Голоса помогают чувствовать себя менее одинокой.',
  },
  {
    key: '20',
    english: "Good idea. I have the TV on, but it's a horror movie. Bad choice! => Haha, definitely bad choice! ",
    russian: 'Хорошая идея. У меня телевизор включён, но это ужастик. Плохой выбор! => Ха-ха, точно плохой выбор!',
  },
  {
    key: '21',
    english: 'Change it to a comedy right now. => Okay, changing now. Also, I checked all the locks three times.',
    russian: 'Срочно переключи на комедию. => Хорошо, переключаю. И я проверила все замки три раза.',
  },
  {
    key: '22',
    english: "Good. You're safe. Nothing will happen. And I'm just a phone call away. => Thanks, Kate. Just hearing your voice helps.",
    russian: 'Хорошо. Ты в безопасности. Ничего не случится. А я всего в одном звонке. => Спасибо, Кейт. Твой голос помогает.',
  },
  {
    key: '23',
    english: 'Do you want me to stay on the phone until you fall asleep? => Would you? I do not want to be annoying.',
    russian: 'Хочешь, я останусь на телефоне, пока не заснёшь? => Побудешь? Не хочу быть навязчивой.',
  },
  {
    key: '24',
    english: "Not annoying at all. I'll talk to you until you're sleepy. Tell me about your day.",
    russian: 'Совсем не навязчиво. Буду говорить, пока не захочешь спать. Расскажи о своём дне.',
  },
];

export const fears_13 = [
  {
    key: '1',
    english: "Ben, I'm so nervous about the move. I can't sleep, I can't eat.",
    russian: 'Бен, я так нервничаю из-за переезда. Не могу спать, не могу есть.',
  },
  {
    key: '2',
    english: "Moving is stressful, but it's also exciting! New city, new job, new friends.",
    russian: 'Переезд волнительный, но и захватывающий! Новый город, новая работа, новые друзья.',
  },
  {
    key: '3',
    english: "That's what scares me. New everything. I hate change.",
    russian: 'Вот что меня пугает. Всё новое. Ненавижу перемены.',
  },
  {
    key: '4',
    english: 'Is it the change itself, or leaving things behind?',
    russian: 'Тебя пугают сами перемены или то, что оставляешь позади?',
  },
  {
    key: '5',
    english: "Both. I'm scared I won't make new friends. I'm scared I'll be lonely.",
    russian: 'И то, и другое. Боюсь, что не найду новых друзей. Боюсь, буду одинока.',
  },
  {
    key: '6',
    english: "You're so friendly! You make friends everywhere. Remember when we met?",
    russian: 'Ты такая дружелюбная! Ты везде находишь друзей. Помнишь, как мы познакомились?',
  },
  {
    key: '7',
    english: 'At that party, right? You talked to me first.',
    russian: 'На той вечеринке, да? Ты первый заговорил.',
  },
  {
    key: '8',
    english: 'Yes, because you looked nice. And we became best friends. It will happen again.',
    russian: 'Да, потому что ты выглядела милой. И мы стали лучшими друзьями. Так будет снова.',
  },
  {
    key: '9',
    english: 'But what if my new job is terrible? What if I hate the city?',
    russian: 'Но что, если новая работа ужасна? Что, если я возненавижу город?',
  },
  {
    key: '10',
    english: "Then you come back. It's not forever. It's an adventure.",
    russian: 'Тогда вернёшься. Это не навсегда. Это приключение.',
  },
  {
    key: '11',
    english: "My mom says the same thing. 'It's an adventure.' But I'm still scared.",
    russian: "Моя мама говорит то же самое. 'Это приключение'. Но мне всё равно страшно.",
  },
  {
    key: '12',
    english: "Fear of change is normal. It's called metathesiophobia, I think.",
    russian: 'Боязнь перемен нормальна. Кажется, это называется метатезиофобия.',
  },
  {
    key: '13',
    english: "There's a name for everything, isn't there?",
    russian: 'Для всего есть название, да?',
  },
  {
    key: '14',
    english: 'Yes! But think about it: every good thing in your life started with change.',
    russian: 'Да! Но подумай: всё хорошее в твоей жизни начиналось с перемен.',
  },
  {
    key: '15',
    english: 'Like what?',
    russian: 'Например?',
  },
  {
    key: '16',
    english: 'Like going to university. That was scary, but you met me and other friends.',
    russian: 'Как поступление в университет. Это было страшно, но ты встретила меня и других друзей.',
  },
  {
    key: '17',
    english: "That's true. I was so scared on the first day of university.",
    russian: 'Это правда. Я так боялась в первый день в университете.',
  },
  {
    key: '18',
    english: "And look at you now. You survived. You thrived. You'll do it again.",
    russian: 'И посмотри на себя сейчас. Ты выжила. Ты процветала. У тебя снова получится.',
  },
  {
    key: '19',
    english: 'Thanks, Ben. You always know what to say. I feel a little braver now.',
    russian: 'Спасибо, Бен. Ты всегда знаешь, что сказать. Теперь я чувствую себя немного храбрее.',
  },
  {
    key: '20',
    english: "Good! Now let's plan your goodbye party!",
    russian: 'Отлично! А теперь давай планировать твою прощальную вечеринку!',
  },
  {
    key: '21',
    english: '--------------------------------------------------------',
    russian: '--------------------------------------------------------',
  },
  {
    key: '22',
    english: 'Jake, can I ask you something? About boys?',
    russian: 'Джейк, могу спросить кое-что? О мальчиках?',
  },
  {
    key: '23',
    english: "Sure, I'll try to help. What's up?",
    russian: 'Конечно, попробую помочь. Что случилось?',
  },
  {
    key: '24',
    english: "There's this boy in my math class. I really like him. I want to ask him out.",
    russian: 'В классе математики есть мальчик. Он мне очень нравится. Хочу пригласить его на свидание.',
  },
  {
    key: '25',
    english: "So? Just ask him. What's the problem?",
    russian: 'И? Просто пригласи. В чём проблема?',
  },
  {
    key: '26',
    english: 'What if he says no? I would be so embarrassed.',
    russian: 'А если он откажет? Мне будет так стыдно.',
  },
  {
    key: '27',
    english: "Ah, you're scared of rejection. That's normal.",
    russian: 'А, ты боишься отказа. Это нормально.',
  },
  {
    key: '28',
    english: 'Is it? I feel so stupid. I think about it all the time.',
    russian: 'Правда? Чувствую себя такой глупой. Постоянно думаю об этом.',
  },
  {
    key: '29',
    english: 'Everyone is scared of rejection, Emma. Even me.',
    russian: 'Все боятся отказа, Эмма. Даже я.',
  },
  {
    key: '30',
    english: "You? But you're so confident!",
    russian: 'Ты? Но ты такой уверенный!',
  },
  {
    key: '31',
    english: "I'm confident sometimes. But when I like someone? Total mess inside.",
    russian: 'Иногда уверен. Но когда кто-то нравится? Внутри полный хаос.',
  },
  {
    key: '32',
    english: 'Really? So what do you do?',
    russian: 'Правда? И что ты делаешь?',
  },
  {
    key: '33',
    english: "I think: what's the worst that can happen? They say no. Then I'm in the same place as before.",
    russian: 'Думаю: что самое худшее может случиться? Мне откажут. И я останусь там же, где и был.',
  },
  {
    key: '34',
    english: "That's true. If I don't ask, he definitely won't go out with me.",
    russian: 'Это правда. Если не спрошу, он точно не пойдёт со мной.',
  },
  {
    key: '35',
    english: "Exactly. If you ask, you have a chance. Maybe 50%. If you don't ask, 0%.",
    russian: 'Именно. Если спросишь, у тебя есть шанс. Может, 50%. Если не спросишь — 0%.',
  },
  {
    key: '36',
    english: "That's good math. I like those odds.",
    russian: 'Хорошая математика. Мне нравятся такие шансы.',
  },
  {
    key: '37',
    english: "And if he says no, it's not the end of the world. There are other boys.",
    russian: 'И если он откажет, это не конец света. Есть и другие мальчики.',
  },
  {
    key: '38',
    english: 'I know. But it will hurt.',
    russian: 'Знаю. Но будет больно.',
  },
  {
    key: '39',
    english: "Yes, it will hurt for a day or two. Then you move on. You're strong.",
    russian: 'Да, будет больно день или два. Потом живёшь дальше. Ты сильная.',
  },
  {
    key: '40',
    english: "Okay. I'm going to do it. Tomorrow. Wish me luck!",
    russian: 'Хорошо. Я сделаю это. Завтра. Пожелай мне удачи!',
  },
  {
    key: '41',
    english: 'Good luck! Let me know what he says. => I will. Thanks, Jake. You are a good friend.',
    russian: 'Удачи! Дай знать, что он ответит. => Хорошо. Спасибо, Джейк. Ты хороший друг.',
  },
];

export const fears_15 = [
  {
    key: '1',
    english: "Graduation is in two months. I'm freaking out.",
    russian: 'Выпускной через два месяца. Я в панике.',
  },
  {
    key: '2',
    english: "Me too. Real life is coming, and I'm not ready.",
    russian: 'Я тоже. Взрослая жизнь приближается, а я не готов.',
  },
  {
    key: '3',
    english: 'What are you most scared about?',
    russian: 'Чего ты боишься больше всего?',
  },
  {
    key: '4',
    english: "Everything. Finding a job, paying bills, being an adult. It's terrifying.",
    russian: 'Всего. Найти работу, платить по счетам, быть взрослым. Это ужасно.',
  },
  {
    key: '5',
    english: "Same. I don't even know what job I want. My parents keep asking me.",
    russian: 'Так же. Я даже не знаю, какую работу хочу. Родители всё спрашивают.',
  },
  {
    key: '6',
    english: "My parents say 'follow your passion.' But what if my passion doesn't pay?",
    russian: "Мои родители говорят 'следуй за мечтой'. Но что, если моя мечта не приносит денег?",
  },
  {
    key: '7',
    english: "Right? I love art, but I can't make money from art. Probably.",
    russian: 'Верно? Я люблю искусство, но на искусстве денег не заработать. Наверное.',
  },
  {
    key: '8',
    english: 'Maybe you can. Graphic design, advertising, teaching art...',
    russian: 'Может, и заработаешь. Графический дизайн, реклама, преподавание...',
  },
  {
    key: '9',
    english: "That's true. There are options. I just don't know which one.",
    russian: 'Это правда. Есть варианты. Просто не знаю, какой выбрать.',
  },
  {
    key: '10',
    english: "I'm scared of making the wrong choice. What if I choose a career and hate it?",
    russian: 'Боюсь сделать неправильный выбор. Что, если выберу карьеру и возненавижу её?',
  },
  {
    key: '11',
    english: 'Then you change it. People change careers all the time now.',
    russian: 'Тогда поменяешь. Люди сейчас часто меняют карьеру.',
  },
  {
    key: '12',
    english: "But it's scary. Everything is uncertain.",
    russian: 'Но это страшно. Всё так неопределённо.',
  },
  {
    key: '13',
    english: 'Fear of the future is normal. Everyone feels it. Even confident people.',
    russian: 'Боязнь будущего нормальна. Все её чувствуют. Даже уверенные люди.',
  },
  {
    key: '14',
    english: "Do you think we'll be okay?",
    russian: 'Думаешь, у нас всё будет хорошо?',
  },
  {
    key: '15',
    english: "Yes. We're smart, we work hard. We'll figure it out.",
    russian: 'Да. Мы умные, мы усердно работаем. Мы разберёмся.',
  },
  {
    key: '16',
    english: 'Together? Like we figured out university together?',
    russian: 'Вместе? Как разобрались с университетом вместе?',
  },
  {
    key: '17',
    english: "Of course. We'll complain about our jobs together over coffee.",
    russian: 'Конечно. Будем жаловаться на работу вместе за кофе.',
  },
  {
    key: '18',
    english: "Haha, deal. That makes me feel better. As long as we're not alone in this.",
    russian: 'Ха-ха, договорились. Мне легче. Главное, что мы не одни.',
  },
  {
    key: '19',
    english: "We're not. We have each other, and our families. We'll be fine.",
    russian: 'Мы не одни. У нас есть друг друг и наши семьи. Всё будет хорошо.',
  },
  {
    key: '20',
    english: "Thanks, Mark. I'm still scared, but less scared now.",
    russian: 'Спасибо, Марк. Мне всё ещё страшно, но уже меньше.',
  },
  {
    key: '21',
    english: "Good. Now let's enjoy our last months of being students!",
    russian: 'Отлично. А теперь давай наслаждаться последними месяцами студенчества!',
  },
  {
    key: '22',
    english: '--------------------------------------------------------',
    russian: '--------------------------------------------------------',
  },
  {
    key: '23',
    english: 'I heard you have a new job. Congratulations!',
    russian: 'Я слышал, у тебя новая работа. Поздравляю!',
  },
  {
    key: '24',
    english: "Thanks! But I'm so nervous. First day is tomorrow.",
    russian: 'Спасибо! Но я так нервничаю. Завтра первый день.',
  },
  {
    key: '25',
    english: "You'll be fine! Are you more afraid of the new job or public speaking?",
    russian: 'Всё будет хорошо! Ты больше боишься новой работы или публичных выступлений?',
  },
  {
    key: '26',
    english: "Good question. I think I'm much more afraid of public speaking.",
    russian: 'Хороший вопрос. Думаю, я гораздо больше боюсь публичных выступлений.',
  },
  {
    key: '27',
    english: 'Really? Why?',
    russian: 'Правда? Почему?',
  },
  {
    key: '28',
    english: "At a new job, I can watch and learn. But public speaking? Everyone looks at me. It's terrifying.",
    russian: 'На новой работе я могу смотреть и учиться. А публичные выступления? Все смотрят на меня. Это ужасно.',
  },
  {
    key: '29',
    english: 'I understand. For me, flying is the most frightening thing. Even more than spiders.',
    russian: 'Понимаю. Для меня полёты — самое страшное. Даже больше, чем пауки.',
  },
  {
    key: '30',
    english: "Wait, you're scared of flying? But you travel so much!",
    russian: 'Подожди, ты боишься летать? Но ты так много путешествуешь!',
  },
  {
    key: '31',
    english: "I know! I'm less scared of flying than before, but still scared. I take medicine.",
    russian: 'Знаю! Я меньше боюсь летать, чем раньше, но всё равно боюсь. Принимаю лекарства.',
  },
  {
    key: '32',
    english: "That's brave. I couldn't do it. Heights are scary enough for me.",
    russian: 'Это храбро. Я бы не смог. Высоты мне достаточно страшно.',
  },
  {
    key: '33',
    english: "So you're more scared of heights than flying?",
    russian: 'Значит, ты больше боишься высоты, чем полётов?',
  },
  {
    key: '34',
    english: "Oh yes! Much more. I can't even look down from a tall building.",
    russian: 'О да! Гораздо больше. Я даже с высокого здания вниз смотреть не могу.',
  },
  {
    key: '35',
    english: 'See? Everyone has different fears. Your new job will be easy compared to heights!',
    russian: 'Видишь? У всех разные страхи. Твоя новая работа будет лёгкой по сравнению с высотой!',
  },
  {
    key: '36',
    english: 'Haha, good point. Thanks! I feel a little better now.',
    russian: 'Ха-ха, хорошая мысль. Спасибо! Мне немного легче.',
  },
];

export const fears_17 = [
  {
    key: '1',
    english: 'Mom, can we have a dog? Please?',
    russian: 'Мам, можно завести собаку? Пожалуйста?',
  },
  {
    key: '2',
    english: "Oh, sweetie. You know I'm scared of dogs. Even small ones.",
    russian: 'Ох, милый. Ты же знаешь, я боюсь собак. Даже маленьких.',
  },
  {
    key: '3',
    english: "But you're more scared of cats! Remember when a cat hissed at you?",
    russian: 'Но ты больше боишься кошек! Помнишь, как кошка на тебя зашипела?',
  },
  {
    key: '4',
    english: "That's true. I am more afraid of cats than dogs. Cats are unpredictable.",
    russian: 'Это правда. Я больше боюсь кошек, чем собак. Кошки непредсказуемы.',
  },
  {
    key: '5',
    english: 'So dogs are less scary for you?',
    russian: 'Значит, собаки для тебя менее страшны?',
  },
  {
    key: '6',
    english: 'A little less scary, yes. But still scary. A dog bit me when I was your age.',
    russian: 'Да, немного менее страшны. Но всё равно страшно. Собака укусила меня, когда я была в твоём возрасте.',
  },
  {
    key: '7',
    english: "I'm sorry, Mom. But what about a small, friendly dog? Like a golden retriever?",
    russian: 'Мне жаль, мам. А как насчёт маленькой дружелюбной собаки? Например, золотистого ретривера?',
  },
  {
    key: '8',
    english: "Golden retrievers are big! They're the most frightening for me because they're so energetic.",
    russian: 'Золотистые ретриверы большие! Они для меня самые страшные, потому что они такие энергичные.',
  },
  {
    key: '9',
    english: 'Okay... what about a tiny dog? Like a chihuahua?',
    russian: 'Ладно... а как насчёт крошечной собачки? Например, чихуахуа?',
  },
  {
    key: '10',
    english: "Hmm. A chihuahua is less frightening than a big dog. They're small and not so strong.",
    russian: 'Хм. Чихуахуа менее страшна, чем большая собака. Они маленькие и не такие сильные.',
  },
  {
    key: '11',
    english: 'So maybe one day?',
    russian: 'Так может, когда-нибудь?',
  },
  {
    key: '12',
    english: "Maybe. But first, let's visit a friend with a small dog. I need to practice being less scared.",
    russian: 'Может быть. Но сначала давай сходим в гости к друзьям с маленькой собакой. Мне нужно тренироваться меньше бояться.',
  },
  {
    key: '13',
    english: "Deal! You're so brave, Mom.",
    russian: 'Договорились! Ты такая храбрая, мам.',
  },
  {
    key: '14',
    english: "I'm trying, sweetie. For you.",
    russian: 'Я стараюсь, милый. Ради тебя.',
  },
  {
    key: '15',
    english: '--------------------------------------------------------',
    russian: '--------------------------------------------------------',
  },
  {
    key: '16',
    english: 'Are you okay? You look pale.',
    russian: 'Ты в порядке? Выглядишь бледной.',
  },
  {
    key: '17',
    english: "I have a doctor's appointment today. Blood test.",
    russian: 'У меня сегодня приём у врача. Анализ крови.',
  },
  {
    key: '18',
    english: 'Ah, needles. Are you scared?',
    russian: 'А, иглы. Боишься?',
  },
  {
    key: '19',
    english: 'Terrified. Needles are my biggest fear. Much more than spiders or heights.',
    russian: 'До ужаса. Иглы — мой самый большой страх. Гораздо больше, чем пауки или высота.',
  },
  {
    key: '20',
    english: 'Really? For me, needles are less scary than going to the dentist.',
    russian: 'Правда? Для меня иглы менее страшны, чем поход к стоматологу.',
  },
  {
    key: '21',
    english: "The dentist? But that's just cleaning!",
    russian: 'Стоматолог? Но это же просто чистка!',
  },
  {
    key: '22',
    english: 'No way. The sound of the drill is the most frightening noise in the world.',
    russian: 'Ни за что. Звук бормашины — самый страшный звук в мире.',
  },
  {
    key: '23',
    english: 'Hmm. I think a needle is worse. It goes inside your skin!',
    russian: 'Хм. А я думаю, игла хуже. Она проникает под кожу!',
  },
  {
    key: '24',
    english: 'For one second. The dentist is 30 minutes of fear!',
    russian: 'На одну секунду. А у стоматолога 30 минут страха!',
  },
  {
    key: '25',
    english: 'Okay, you win. The dentist might be worse. But needles are still terrifying for me.',
    russian: 'Ладно, ты победила. Стоматолог, может, и хуже. Но иглы для меня всё равно ужасны.',
  },
  {
    key: '26',
    english: "What's the least scary thing for you?",
    russian: 'А что для тебя наименее страшно?',
  },
  {
    key: '27',
    english: "Probably the dark. I'm not scared of the dark at all. My sister is, but I'm not.",
    russian: 'Наверное, темнота. Я совсем не боюсь темноты. Моя сестра боится, а я нет.',
  },
  {
    key: '28',
    english: "See? Everyone's different. I'm more scared of the dark than needles!",
    russian: 'Видишь? У всех по-разному. Я больше боюсь темноты, чем игл!',
  },
  {
    key: '29',
    english: "Really? That's funny. Okay, I feel a little better now. Talking helps.",
    russian: 'Правда? Забавно. Ладно, мне немного лучше. Разговоры помогают.',
  },
  {
    key: '30',
    english: "Good luck at the doctor! You'll be fine. Just don't look at the needle.",
    russian: 'Удачи у врача! Всё будет хорошо. Просто не смотри на иглу.',
  },
];
